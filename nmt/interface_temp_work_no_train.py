...
def add_arguments(parser,model_dir):
  ...
  # data
  parser.add_argument("--src", type=str, default="cor",
                      help="Source suffix, e.g., en.")
  parser.add_argument("--tgt", type=str, default="man",
...
###########change out_dir default for interface
  parser.add_argument("--out_dir", type=str, default=model_dir,
                      help="Store log/model files.")

  # Vocab
  parser.add_argument("--vocab_prefix", type=str, default=os.path.join(model_dir, "vocab"), help="""\
  ...
  ###########change inf_inp default for interface
  parser.add_argument("--inference_input_file", type=str, default=' ',
  ...
  ###########change inf_out default for interface
...
def create_or_load_hparams(out_dir, default_hparams, hparams_path):
  ...
  print("Create hparams or load hparams from out_dir.")
  print(out_dir)
  ...
  for metric in hparams.metrics:
    utils.save_hparams(os.path.join(out_dir, 'best_bleu'), hparams)
...
def run_main(flags, default_hparams, train_fn, inference_fn, target_session="", msg =''):
  """Run main."""
  # Jobuua
  jobid =  flags.jobid
  ...
  ## Train / Decode
  out_dir = param['model_dir'] # out_dir = flags.out_dir
...
def init(model_dir):
  ...
  add_arguments(nmt_parser,model_dir)
  param['flags'], unparsed = nmt_parser.parse_known_args()
  param['default_hparams'] = create_hparams(param['flags'])
  param['train_fn'] =  train.train
  param['model_dir'] =  model_dir
  param['inference_fn'] = inference.inference

def send(msg=''):
  return run_main(param['flags'], param['default_hparams'], param['train_fn'], param['inference_fn'], msg=msg)

def main(unused_argv):
  default_hparams = create_hparams(FLAGS)
  train_fn = train.train
  inference_fn = inference.inference
  return run_main(FLAGS, default_hparams, train_fn, inference_fn)
